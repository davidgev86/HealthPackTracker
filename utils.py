...
def generate_hpm_shopping_list_pdf(hpm_low_stock_items: List[InventoryItem]) -> bytes:
    """Generate shopping list PDF for HPM low stock items"""
    buffer = BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=letter)
    styles = getSampleStyleSheet()
    story = []
    ...
    story.append(Paragraph("Health Pack Meals - HPM Shopping List", title_style))
    story.append(Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", styles['Normal']))
    story.append(Spacer(1, 20))
    
    if not hpm_low_stock_items:
        story.append(Paragraph("No HPM items are currently low in stock.", styles['Normal']))
    else:
        # Group items by category
        category_groups = {}
        for item in hpm_low_stock_items:
        ...
        for category, items in sorted(category_groups.items()):
            ...
            # Items table
            table_data = [['Item', 'Current Stock', 'Par Level', 'Needed', 'Unit Cost', 'Total Cost']]
            ...
            for item in items:
                ...
                table_data.append([
                    ...
                    f"{item.par_level} {item.unit}",
                    f"{needed:.1f} {item.unit}",
                    f"${item.unit_cost:.2f}",
                    f"${total_cost:.2f}"
                ])
        ...
        # Summary
        total_needed_cost = sum(item.quantity_needed() * item.unit_cost for item in hpm_low_stock_items)
        ...
        story.append(Paragraph(f"<b>Total Estimated Cost for HPM Items: ${total_needed_cost:.2f}</b>", summary_style))
        story.append(Paragraph(f"<b>Total HPM Items to Restock: {len(hpm_low_stock_items)}</b>", summary_style))
